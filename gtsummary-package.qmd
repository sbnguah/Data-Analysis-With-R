...
```{r comment=NA}
df_cint_all <- dget("cint_data_clean")
df_mbu <- readxl::read_xlsx("mbu.xlsx")
```
...
df_cint <- 
    ...
    mutate(bmicat = case_when(bmi < 25 ~ "Normal",
                              bmi > 25 ~ "High") %>% 
...
df_mbu_clean <-
    ...
    drop_na() %>% 
    labelled::set_variable_labels(wt = "Weight (kgs)",
                                  sex = "Sex",
                                  age = "Age (days)",
                                  apgar1 = "APGAR (min 1)",
                                  apgar5 = "APGAR (min 5)",
...
df_mbu_clean %>% summarytools::dfSummary(graph.col = F)
```

# Summarizing data

```{r comment=NA}
df_cint %>% summarytools::dfSummary(graph.col = F, labels.col = F)
```
...
[^gtsummary-package-1]: Need to insert this k3k3
